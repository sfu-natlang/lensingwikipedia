...
def join_keywords(keywords):
    return ','.join(k.strip().replace('\t', ' ').replace(',', '\t') for k in keywords)
...
class TextQueryParser(whoosh.qparser.QueryParser):
    ...
    def parse(self, text):
        ...
        # Note that commas here will already be interpreted by the query parser
        # as whitespace, so we don't need to escape them.
        def alter(node):
            new_node = node
...
def split_keywords(value):
    if isinstance(value, unicode):
        if len(value) == 0:
            ...
            return [p.replace('\t', ',') for p in value.split(',')]
    else:
        return [value]

def copy_all(input_index, output_writer, modify_doc):
    """
    Copies all documents in a index with updates made by a modify function that
    is given each document and changes it in-place.
    """
...
def update_all_in_place(index, writer, modify_doc, order_num_field, buffer_size=100):
    """
    Updates all documents in an index in-place. Requires a numeric field which
    is unique (to avoid issues with reading and writing at the same time), and
    buffers some number of documents in memory at once. Otherwise should work
    like copy_all().
    """
    with writer.searcher() as searcher:
        ...
        while True:
            ...
            range_ = whoosh.query.NumericRange(order_num_field,
                                               page_start,
                                               page_start + buffer_size - 1)
            hits = searcher.search(range_, limit=None, sortedby=order_num_field)
            if hits.is_empty():
                break
