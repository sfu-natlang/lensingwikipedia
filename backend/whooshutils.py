...
comma_rep_char_re = re.compile(re.escape(comma_rep_char))
...
def join_keywords(keywords):
  return comma_char.join(comma_re.sub("_", k.strip()) for k in keywords)

# Escape keywords without merging them into a single field value
def escape_keywords(keywords):
  return (comma_re.sub("_", k.strip()) for k in keywords)
...
def split_keywords(value):
  if isinstance(value, unicode):
    if len(value) == 0:
      ...
      return [comma_rep_char_re.sub(comma_char, p) for p in value.split(comma_char)]
  else:
    return [value]
